...
bool pre_offset = false;
bool size_offset = false;
bool tool_ispen = false;
...
static int ft5x0x_read_Touchdata(struct ft5x0x_ts_data *data)
{
	...
	int i = 0;
	//u8 j = 0;
	int FT_STEP = FT_TOUCH_STEP;
	u8 pointid = FT_MAX_ID;
	u8 icmode;
	u16 ft_width,ft_height;
	...
	if((size_offset) && (!pre_offset))
		FT_STEP = FT_TOUCH_STEP + FT_TOUCH_SIZE_OFFSET;
	else if((!size_offset) && (pre_offset))
		FT_STEP = FT_TOUCH_STEP + FT_TOUCH_PRE_OFFSET;
	else if(size_offset && pre_offset)
		FT_STEP = FT_TOUCH_STEP + 4;
	else FT_STEP = FT_TOUCH_STEP;
		
	for (i = 0; i < CFG_MAX_TOUCH_POINTS; i++) {
		pointid = (buf[FT_TOUCH_ID_POS + FT_STEP * i]) >> 4;
		if (pointid >= FT_MAX_ID)
		break;
		else
			event->touch_point++;
		event->au16_x[i] =
		    (s16) (buf[FT_TOUCH_X_H_POS + FT_STEP * i] & 0x0F) <<
		    8 | (s16) buf[FT_TOUCH_X_L_POS + FT_STEP * i];
		event->au16_y[i] =
		    (s16) (buf[FT_TOUCH_Y_H_POS + FT_STEP * i] & 0x0F) <<
		    8 | (s16) buf[FT_TOUCH_Y_L_POS + FT_STEP * i];
		event->au8_touch_event[i] =
		    buf[FT_TOUCH_EVENT_POS + FT_STEP * i] >> 6;
		event->au8_finger_id[i] =
		    (buf[FT_TOUCH_ID_POS + FT_STEP * i]) >> 4;
		if(pre_offset)
		{
			event->pressure[i] = 
			(s16)(buf[FT_TOUCH_PRESSURE_H+ FT_STEP * i] & 0xFF) <<
			2 | (s16)((buf[FT_TOUCH_PRESSURE_L+ FT_STEP * i] & 0xC0)>> 6);
		}
		else event->pressure[i] = 0x3f;
		if(size_offset)
		{
			ft_width = 
			(s16)(buf[FT_TOUCH_WIDTH_H+ FT_STEP * i] & 0x3F) <<
			4 | (s16)((buf[FT_TOUCH_WIDTH_L+ FT_STEP * i] & 0xF0) >> 4);

			ft_height = 
			(s16)(buf[FT_TOUCH_HEIGHT_H+ FT_STEP * i] & 0x03) <<
			8 | (s16)(buf[FT_TOUCH_HEIGHT_L+ FT_STEP * i] & 0xFF) ;

			event->area[i] = (ft_width * ft_width + ft_height * ft_height)/(ft_width + ft_height + 1);
		}
			else event->area[i] = 0x05;
	}
			   ...
			   const struct i2c_device_id *id)
{
	...
	ft5x0x_ts->x_max = 1280 - 1;
	ft5x0x_ts->y_max = 800 - 1;

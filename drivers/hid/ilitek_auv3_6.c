...
#define ILITEK_ERROR_LEVEL		KERN_ALERT
	...
	#define USB_DEVICE_ID_ILITEK_MT	0x0064
#endif
...
static int 
ilitek_register_input_device(
	struct input_dev **input, int support_touch_point, 
	int max_x, int max_y, char* name)
{
 ...
 #if LINUX_VERSION_CODE >= KERNEL_VERSION(3,0,8)
static void hid_feature_mapping(struct hid_device *hdev,
	...
	switch(usage->hid & HID_USAGE_PAGE){
		case HID_UP_GENDESK:
			switch(usage->hid){
				...
				case HID_GD_Y:
					...
					printk(ILITEK_DEBUG_LEVEL "%s, max_x %d, max_y %d\n", __func__, ilitek.hid.max_x, ilitek.hid.max_y);

					if(ilitek.hid.valid_input_register == 0){
						int ret = ilitek_register_input_device(&(ilitek.hid.input_dev), ilitek.hid.support_touch_point,
		...
		case HID_UP_DIGITIZER:
			...
			case HID_DG_CONTACTID:
					...
					input_mt_init_slots(hi->input,ilitek.hid.support_touch_point, 0);
				#else
...
static int 
hid_event(
	struct hid_device *hid, struct hid_field *field, 
...
static int 
hid_probe(
	struct hid_device *hdev, const struct hid_device_id *dev_id)
{
	int ret;
	struct hid_report *report;
...
{
	printk(ILITEK_DEBUG_LEVEL "%s, remove hid device\n", __func__);
...
ilitek_input_report(
    struct input_dev *input_dev,
    struct touch_data *tp, int tpnum)
{
	int i,ispressed = 0;
	DBG("%s,enter,tpnum=%d\n",__func__,tpnum);
	if(tpnum == 1) {

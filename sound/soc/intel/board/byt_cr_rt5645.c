...
static int byt_hs_detection(void)
{
	...
	if (!status) {
		pr_debug("Jack insert intr");
		rt5645_headset_detect(codec, true);
		jack_type = SND_JACK_HEADPHONE;
	} else {
		pr_debug("Jack out");
		rt5645_headset_detect(codec, false);
		jack_type = 0;
	}
	jack->status = jack_type;
#if 0
	if (!jack->status) {
...
static inline struct snd_soc_codec *byt_get_codec(struct snd_soc_card *card)
{
	...
	list_for_each_entry(codec, &card->codec_dev_list, card_list) {
		if (!strstr(codec->name, "rt5645.2-001a")) {
			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	return byt_set_dai_fmt_pll(codec_dai, fmt, RT5645_PLL1_S_MCLK,
						params_rate(params));

}

#if 0 /* for CR platform */
			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	fmt = SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_NB_NF |
	      SND_SOC_DAIFMT_CBS_CFS;
	ret = byt_set_dai_fmt_pll(codec_dai, fmt, RT5645_PLL1_S_MCLK,
						params_rate(params));
	if (ret < 0) {
...
static int byt_compr_set_params(struct snd_compr_stream *cstream)
{
	...
	fmt = SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_NB_NF |
	      SND_SOC_DAIFMT_CBS_CFS;
	return byt_set_dai_fmt_pll(codec_dai, fmt, RT5645_PLL1_S_MCLK, 48000);
}
...
static int byt_init(struct snd_soc_pcm_runtime *runtime)
{
	...
	if (INTEL_MID_BOARD(3, TABLET, BYT, BLK, PRO, 8PR1))
		...
		rt5645_config_ovcd_thld(codec, RT5645_MIC1_OVTH_1500UA,
					RT5645_MIC_OVCD_SF_1P0);
	else
		/* Threshold base = 2000uA; scale factor = 0.5 =>
		effective threshold of 1000uA */
		rt5645_config_ovcd_thld(codec, RT5645_MIC1_OVTH_2000UA,
					RT5645_MIC_OVCD_SF_0P5);
	...
	snd_soc_dapm_nc_pin(dapm, "IN1P");
	snd_soc_dapm_nc_pin(dapm, "IN1N");
...
{
	pr_info("Baytrail Machine Driver byt_rt5645 registerd\n");
